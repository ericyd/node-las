  ...
  LAS: [
    'object',
    {
      ...
      pointData: jBinary.Type({
        read: function(header) {
          return this.binary.seek(header.offsetToPoints, function() {
      ...
      extendedVariableLengthRecords: jBinary.Type({
        read: function(header) {
          ...
          return this.binary.seek(header.startOfEVLRs, function() {
            const evlrHeader = this.binary.getType('evlrHeader');
            const records = [];
            for (i = 0; i < header.numberOfEVLRs; i++) {
