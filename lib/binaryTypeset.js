      ...
      pointData: jBinary.Type({
        read: function(header) {
          ...
          return this.binary.seek(header.offsetToPointData, function() {
            const pointData = this.getType(`pointDataRecordFormat${header.pointDataRecordFormat}`);
            // console.log(pointData)
            const records = [];
            for (i = 0; i < header.numberOfPointRecords; i++) {
      ...
      extendedVariableLengthRecords: jBinary.Type({
        read: function(header) {
          return this.binary.seek(header.startOfFirstExtendedVariableLengthRecord, function() {
            ...
            const records = [];
            for (i = 0; i < header.numberOfExtendedVariableLengthRecords; i++) {
              const evlrHeaderValues = this.read(evlrHeader);
              const EVLR = ['string', evlrHeaderValues.recordLengthAfterHeader];
